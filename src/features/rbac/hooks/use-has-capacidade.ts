 ...
 */
export function useHasCapacidade(userId: number | undefined, tenantId: string | undefined) {
  const enabled = Boolean(userId && tenantId);

  const query = useQuery({
    queryKey: ["rbac", "user-caps", userId, tenantId ?? null],
    queryFn: () => obterCapacidadesEfetivas(userId!, tenantId!),
    enabled,
    staleTime: 60_000,
  });
