...
import { buildDefaultCrmPipelineStages } from "../crm/workspace";
import { createSessaoAulaId, getNextOccurrenceForDiasSemana } from "../aulas/reservas";
...
interface Store {
  ...
  nfseConfiguracoes: NfseConfiguracao[];
  agregadorTransacoes: AgregadorTransacao[];
  integracoesOperacionais: IntegracaoOperacional[];
  ...
  crmTasks: CrmTask[];
  crmPlaybooks: CrmPlaybook[];
  crmCadencias: CrmCadencia[];
...
function applyLegacyTenantMigration(input: Store): Store {
  ...
  const result: Store = {
    ...
    nfseConfiguracoes: mapTenantItems(input.nfseConfiguracoes),
    agregadorTransacoes: mapTenantItems(input.agregadorTransacoes),
    integracoesOperacionais: mapTenantItems(input.integracoesOperacionais),
    ...
    crmPipelineStages: mapTenantItems(input.crmPipelineStages),
    crmTasks: mapTenantItems(input.crmTasks),
    crmPlaybooks: mapTenantItems(input.crmPlaybooks),
    crmCadencias: mapTenantItems(input.crmCadencias),
    crmAutomations: mapTenantItems(input.crmAutomations),
    crmActivities: mapTenantItems(input.crmActivities),
    reservasAulas: mapTenantItems(input.reservasAulas),
...
function makeInitialStore(): Store {
  ...
  const crmPipelineStages: CrmPipelineStage[] = [
    ...buildDefaultCrmPipelineStages(TENANT_ID),
    ...buildDefaultCrmPipelineStages(TENANT_ID_S1),
    ...buildDefaultCrmPipelineStages(TENANT_ID_S3),
  ];

  const crmPlaybooks: CrmPlaybook[] = [
    {
      id: "cpb-s1-001",
      tenantId: TENANT_ID_S1,
      nome: "Visita experimental com fechamento",
      objetivo: "Padronizar o handoff entre visita e proposta comercial.",
      stageStatus: "AGENDOU_VISITA",
      ativo: true,
      passos: [
        {
          id: "cpb-s1-001-step-1",
          titulo: "Confirmar presença até 2h antes",
          descricao: "Enviar mensagem de confirmação com orientações de chegada.",
          acao: "SCRIPT_WHATSAPP",
          prazoHoras: 2,
          obrigatoria: true,
        },
        {
          id: "cpb-s1-001-step-2",
          titulo: "Apresentar planos logo após o tour",
          descricao: "Usar proposta mensal e plano trimestral como default.",
          acao: "PROPOSTA",
          prazoHoras: 4,
          obrigatoria: true,
        },
    ...
    },
    {
      id: "cpb-s3-001",
      tenantId: TENANT_ID_S3,
      nome: "Qualificação rápida inbound",
      objetivo: "Qualificar lead inbound em até 24h sem perder timing.",
      stageStatus: "NOVO",
      ativo: true,
      passos: [
        {
          id: "cpb-s3-001-step-1",
          titulo: "Mapear objetivo principal",
          descricao: "Registrar meta do aluno e principal objeção.",
          acao: "CHECKLIST",
          prazoHoras: 6,
          obrigatoria: true,
        },
        {
          id: "cpb-s3-001-step-2",
          titulo: "Fazer ligação se não responder no WhatsApp",
          acao: "LIGACAO",
          prazoHoras: 18,
          obrigatoria: false,
        },
      ],
      dataCriacao: "2026-02-16T08:30:00",
    },
  ];

  const crmCadencias: CrmCadencia[] = [
    {
      id: "ccd-s1-001",
      tenantId: TENANT_ID_S1,
      nome: "Cadência lead novo 3 dias",
      objetivo: "Garantir primeira resposta e tentativa de agenda em 72h.",
      stageStatus: "NOVO",
      gatilho: "NOVO_PROSPECT",
      ativo: true,
      passos: [
        {
          id: "ccd-s1-001-step-1",
          titulo: "Mensagem de boas-vindas",
          acao: "WHATSAPP",
          delayDias: 0,
          template: "Oi {{nome}}, posso te ajudar a encontrar o melhor plano para seu objetivo?",
          automatica: true,
        },
        {
          id: "ccd-s1-001-step-2",
          titulo: "Ligação de tentativa",
          acao: "LIGACAO",
          delayDias: 1,
          template: "Confirmar interesse e sugerir visita experimental.",
          automatica: false,
        },
        {
          id: "ccd-s1-001-step-3",
          titulo: "Criar follow-up final",
          acao: "TAREFA_INTERNA",
          delayDias: 3,
          template: "Revisar objeções e decidir reciclagem ou perda.",
          automatica: true,
        },
      ],
      ultimaExecucao: "2026-03-09T10:00:00",
      dataCriacao: "2026-02-19T09:30:00",
      dataAtualizacao: "2026-03-09T10:00:00",
    },
    {
      id: "ccd-s3-001",
      tenantId: TENANT_ID_S3,
      nome: "Pós-visita sem resposta",
      objetivo: "Retomar contato com quem visitou e ainda não fechou plano.",
      stageStatus: "VISITOU",
      gatilho: "VISITA_REALIZADA",
      ativo: true,
      passos: [
        {
          id: "ccd-s3-001-step-1",
          titulo: "Enviar proposta resumida",
          acao: "EMAIL",
          delayDias: 0,
          template: "Resumo do plano indicado e condições da semana.",
          automatica: true,
        },
        {
          id: "ccd-s3-001-step-2",
          titulo: "Ligar no dia seguinte",
          acao: "LIGACAO",
          delayDias: 1,
          template: "Entender objeção e sugerir próximo passo.",
          automatica: false,
        },
      ],
      dataCriacao: "2026-02-21T14:00:00",
    },
  ];

  const crmAutomations: CrmAutomation[] = [
    {
      id: "cat-s1-001",
      tenantId: TENANT_ID_S1,
      nome: "Lead novo inicia cadência padrão",
      descricao: "Ao criar prospect, dispara a cadência de abordagem inicial.",
      gatilho: "PROSPECT_CRIADO",
      acao: "INICIAR_CADENCIA",
      stageStatus: "NOVO",
      ativo: true,
      execucoes: 18,
      ultimaExecucao: "2026-03-09T10:00:00",
      cadenceId: "ccd-s1-001",
      dataCriacao: "2026-02-19T09:30:00",
      dataAtualizacao: "2026-03-09T10:00:00",
    },
    {
      id: "cat-s1-002",
      tenantId: TENANT_ID_S1,
      nome: "Tarefa atrasada notifica responsável",
      descricao: "Notifica o consultor quando uma tarefa expira sem conclusão.",
      gatilho: "TAREFA_ATRASADA",
      acao: "NOTIFICAR_RESPONSAVEL",
      ativo: true,
      execucoes: 6,
      ultimaExecucao: "2026-03-08T18:20:00",
      dataCriacao: "2026-02-25T08:00:00",
    },
    {
      id: "cat-s3-001",
      tenantId: TENANT_ID_S3,
      nome: "Mudança de etapa aplica playbook",
      descricao: "Ao avançar para visita, aplica playbook de fechamento.",
      gatilho: "ETAPA_ALTERADA",
      acao: "APLICAR_PLAYBOOK",
      stageStatus: "AGENDOU_VISITA",
      ativo: false,
      execucoes: 4,
      playbookId: "cpb-s3-001",
      dataCriacao: "2026-02-22T10:00:00",
    },
  ];
  ...
  const crmTasks: CrmTask[] = [
    {
      id: "ctk-s1-001",
      tenantId: TENANT_ID_S1,
      prospectId: "pr-s1-001",
      prospectNome: "Clara Matos",
      stageStatus: "AGENDOU_VISITA",
      titulo: "Confirmar visita experimental",
      descricao: "Reforçar horário e enviar localização da unidade.",
      tipo: "WHATSAPP",
      prioridade: "ALTA",
      status: "PENDENTE",
      responsavelId: "fn-s1-003",
      responsavelNome: "Diego Paes",
      origem: "CADENCIA",
      vencimentoEm: "2026-03-11T11:00:00",
      dataCriacao: "2026-03-10T09:00:00",
    },
    {
      id: "ctk-s1-002",
      tenantId: TENANT_ID_S1,
      prospectId: "pr-demo-001",
      prospectNome: "Bruna Monteiro Prospect 1",
      stageStatus: "NOVO",
      titulo: "Primeiro contato após lead inbound",
      tipo: "LIGACAO",
      prioridade: "MEDIA",
      status: "EM_ANDAMENTO",
      responsavelId: "fn-s1-003",
      responsavelNome: "Diego Paes",
      origem: "AUTOMACAO",
      vencimentoEm: "2026-03-10T15:00:00",
      dataCriacao: "2026-03-09T17:30:00",
    },
    {
      id: "ctk-s1-003",
      tenantId: TENANT_ID_S1,
      prospectId: "pr-demo-004",
      prospectNome: "Pedro Lima Prospect 4",
      stageStatus: "EM_CONTATO",
      titulo: "Enviar proposta resumida",
      tipo: "PROPOSTA",
      prioridade: "ALTA",
      status: "PENDENTE",
      responsavelId: "fn-s1-001",
      responsavelNome: "Rafaela Nunes",
      origem: "MANUAL",
      vencimentoEm: "2026-03-09T16:30:00",
      dataCriacao: "2026-03-08T18:00:00",
    },
    {
      id: "ctk-s3-001",
      tenantId: TENANT_ID_S3,
      prospectId: "pr-s3-001",
      prospectNome: "Mateus Castro",
      stageStatus: "EM_CONTATO",
      ...
      responsavelNome: "Julia Santos",
      origem: "MANUAL",
      vencimentoEm: "2026-03-11T14:00:00",
  ...
  const crmActivities: CrmActivity[] = [
    {
      ...
      titulo: "Follow-up registrado após agendamento",
      descricao: "Clara confirmou interesse em visita acompanhada.",
      actorNome: "Diego Paes",
      actorId: "fn-s1-003",
      origem: "OPERADOR",
      dataCriacao: "2026-03-10T09:15:00",
    },
    {
      id: "cact-s1-002",
      tenantId: TENANT_ID_S1,
      ...
      taskId: "ctk-s1-001",
      tipo: "TAREFA_CRIADA",
      titulo: "Tarefa criada pela cadência padrão",
      descricao: "Confirmação de visita programada para hoje.",
      actorNome: "Automação CRM",
      origem: "AUTOMACAO",
      dataCriacao: "2026-03-10T09:00:00",
    },
    {
      ...
      titulo: "Cadência lead novo 3 dias executada",
      descricao: "18 execuções na última semana.",
    ...
    },
    {
      ...
      prospectNome: "Mateus Castro",
      tipo: "ETAPA_ALTERADA",
      titulo: "Prospect avançou para Em contato",
      descricao: "Lead respondeu ao primeiro contato pelo Instagram.",
      actorNome: "Julia Santos",
      actorId: "fn-s3-003",
      origem: "OPERADOR",
      dataCriacao: "2026-03-10T10:10:00",
    },
  ];
  ...
  const nfseConfiguracoes: NfseConfiguracao[] = [
    {
      id: "nfse-s1-001",
      tenantId: TENANT_ID_S1,
      ...
      emailCopiaFinanceiro: "fiscal@mananciais-s1.com.br",
      certificadoAlias: "A1-mananciais",
      webhookFiscalUrl: "https://hooks.conceitofit.local/fiscal/mananciais",
      status: "CONFIGURADA",
      ultimaValidacaoEm: "2026-03-09T09:40:00",
      ultimaSincronizacaoEm: "2026-03-10T07:15:00",
    },
    {
      id: "nfse-s3-001",
      tenantId: TENANT_ID_S3,
      ...
      emailCopiaFinanceiro: "fiscal@pechincha-s3.com.br",
      certificadoAlias: "A1-pechincha",
      status: "PENDENTE",
      ultimoErro: "Inscrição municipal pendente de homologação.",
    },
  ];

  const agregadorTransacoes: AgregadorTransacao[] = [
    {
      id: "agt-s1-001",
      tenantId: TENANT_ID_S1,
      pagamentoId: "pg-s1-001",
      adquirente: "STONE",
      maquininhaNome: "POS Recepção",
      nsu: "000981",
      autorizacao: "A5512",
      bandeira: "Visa",
      meioCaptura: "POS",
      ...
      valorLiquido: 126.4,
      parcelas: 1,
      dataTransacao: "2026-02-04T08:20:00",
      dataPrevistaRepasse: "2026-02-05",
      dataRepasse: "2026-02-05",
    ...
    },
    {
      id: "agt-s1-002",
      tenantId: TENANT_ID_S1,
      pagamentoId: "pg-demo-012",
      adquirente: "STONE",
      maquininhaNome: "POS Studio",
      nsu: "000982",
      autorizacao: "A5513",
      bandeira: "Mastercard",
      meioCaptura: "POS",
      clienteNome: "Gustavo Pereira 12",
      descricao: "Mensalidade S1 – Gustavo Pereira 12",
      valorBruto: 129.9,
      taxa: 3.89,
      valorLiquido: 126.01,
      parcelas: 1,
      dataTransacao: "2026-02-13T10:30:00",
      dataPrevistaRepasse: "2026-02-14",
      statusTransacao: "CAPTURADA",
      statusRepasse: "EM_TRANSITO",
      statusConciliacao: "PENDENTE",
      observacao: "Aguardando arquivo de repasse D+1.",
    },
    {
      id: "agt-s1-003",
      tenantId: TENANT_ID_S1,
      pagamentoId: "pg-demo-021",
      adquirente: "CIELO",
      maquininhaNome: "Link Checkout",
      nsu: "000983",
      autorizacao: "A5514",
      bandeira: "Elo",
      meioCaptura: "LINK_PAGAMENTO",
      clienteNome: "Juliana Lima 21",
      descricao: "Mensalidade S1 – Juliana Lima 21",
      valorBruto: 129.9,
      taxa: 4.49,
      valorLiquido: 125.41,
      parcelas: 1,
      dataTransacao: "2026-02-22T15:10:00",
      dataPrevistaRepasse: "2026-02-24",
      statusTransacao: "CAPTURADA",
    ...
    },
    {
      ...
      bandeira: "Visa",
      meioCaptura: "POS",
      clienteNome: "Vitor Carvalho 28",
      descricao: "Mensalidade S1 – Vitor Carvalho 28",
      valorBruto: 119.9,
      taxa: 3.12,
      valorLiquido: 116.78,
      parcelas: 1,
      dataTransacao: "2026-02-28T11:45:00",
      dataPrevistaRepasse: "2026-03-03",
      statusTransacao: "CAPTURADA",
      statusRepasse: "DIVERGENTE",
      statusConciliacao: "DIVERGENTE",
      observacao: "Valor liquidado divergente do extrato do adquirente.",
    },
    {
      id: "agt-s3-001",
      tenantId: TENANT_ID_S3,
      pagamentoId: "pg-s3-001",
      adquirente: "GETNET",
      maquininhaNome: "POS S3",
      nsu: "000991",
      autorizacao: "B2101",
      bandeira: "Visa",
      meioCaptura: "POS",
      clienteNome: "Thiago Prado",
      descricao: "Mensalidade Fevereiro 2026 – Mensal S3",
      valorBruto: 119.9,
      taxa: 3.2,
      valorLiquido: 116.7,
      parcelas: 1,
      dataTransacao: "2026-02-07T09:05:00",
      dataPrevistaRepasse: "2026-02-08",
      dataRepasse: "2026-02-09",
      statusTransacao: "CAPTURADA",
      statusRepasse: "LIQUIDADO",
      statusConciliacao: "CONCILIADA",
    },
  ...
  const integracoesOperacionais: IntegracaoOperacional[] = [
    {
      id: "int-s1-nfse",
      tenantId: TENANT_ID_S1,
      ...
      filaPendente: 0,
      latenciaMs: 240,
      ultimaExecucaoEm: "2026-03-10T07:15:00",
      ultimaSucessoEm: "2026-03-10T07:15:00",
      linkDestino: "/administrativo/nfse",
      ocorrencias: [
        {
          id: "int-s1-nfse-occ-1",
          integracaoId: "int-s1-nfse",
          severidade: "INFO",
          mensagem: "Lote fiscal validado com sucesso.",
          dataCriacao: "2026-03-10T07:15:00",
        },
      ],
    },
    {
      id: "int-s1-stone",
      tenantId: TENANT_ID_S1,
      nome: "Repasse adquirente",
      tipo: "ADQUIRENTE",
      fornecedor: "Stone",
      status: "ATENCAO",
      filaPendente: 2,
      latenciaMs: 620,
      ultimaExecucaoEm: "2026-03-10T08:00:00",
      ultimaSucessoEm: "2026-03-10T07:52:00",
      ultimoErro: "2 transações aguardando confirmação de repasse.",
      linkDestino: "/gerencial/agregadores",
      ocorrencias: [
        {
          id: "int-s1-stone-occ-1",
          integracaoId: "int-s1-stone",
          severidade: "WARN",
          mensagem: "Arquivo D+1 ainda não conciliado para POS Studio.",
          codigo: "STONE_REPASSE_DELAY",
          dataCriacao: "2026-03-10T08:00:00",
        },
      ],
    },
    {
      id: "int-s1-catraca",
      tenantId: TENANT_ID_S1,
      nome: "Catraca websocket",
      tipo: "CATRACA",
      fornecedor: "Control iD",
      status: "SAUDAVEL",
      filaPendente: 0,
      latenciaMs: 90,
      ultimaExecucaoEm: "2026-03-10T08:04:00",
      ultimaSucessoEm: "2026-03-10T08:04:00",
      linkDestino: "/administrativo/catraca-status",
      ocorrencias: [
        {
          id: "int-s1-catraca-occ-1",
          integracaoId: "int-s1-catraca",
          severidade: "INFO",
          mensagem: "Canal websocket reconectado sem perdas.",
          dataCriacao: "2026-03-10T08:04:00",
        },
      ],
    },
    {
      ...
      tipo: "WEBHOOK",
      fornecedor: "Automation Hub",
      status: "FALHA",
      filaPendente: 7,
      latenciaMs: 1400,
      ultimaExecucaoEm: "2026-03-10T08:10:00",
      ultimaSucessoEm: "2026-03-09T20:30:00",
      ultimoErro: "Timeout ao entregar confirmação de venda.",
      linkDestino: "/administrativo/integracoes",
      ocorrencias: [
        {
          id: "int-s1-webhook-occ-1",
          integracaoId: "int-s1-webhook",
          severidade: "ERROR",
          mensagem: "Timeout ao entregar confirmação de venda.",
          codigo: "HOOK_TIMEOUT",
          dataCriacao: "2026-03-10T08:10:00",
        },
        {
          id: "int-s1-webhook-occ-2",
          integracaoId: "int-s1-webhook",
          severidade: "WARN",
          mensagem: "Fila de reprocessamento ultrapassou 5 eventos.",
          dataCriacao: "2026-03-10T08:05:00",
        },
      ],
    },
    {
      ...
      ocorrencias: [
        {
          id: "int-s3-nfse-occ-1",
          integracaoId: "int-s3-nfse",
          severidade: "WARN",
          mensagem: "Inscrição municipal não homologada.",
          dataCriacao: "2026-03-08T15:20:00",
        },
      ],
    },
    {
      ...
      filaPendente: 4,
      latenciaMs: 1800,
      ultimaExecucaoEm: "2026-03-10T06:50:00",
      ultimaSucessoEm: "2026-03-09T21:15:00",
      ultimoErro: "API do adquirente respondeu 503.",
      linkDestino: "/gerencial/agregadores",
      ocorrencias: [
        {
          id: "int-s3-stone-occ-1",
          integracaoId: "int-s3-stone",
          severidade: "ERROR",
          mensagem: "API do adquirente respondeu 503.",
          codigo: "GETNET_503",
          dataCriacao: "2026-03-10T06:50:00",
        },
      ],
    },
  ];
  ...
  const nextS1ClassDate = getNextOccurrenceForDiasSemana(["SEG", "QUA", "SEX"]);
  const nextS3ClassDate = getNextOccurrenceForDiasSemana(["TER", "QUI"]);
  const s1SessaoId = createSessaoAulaId("agr-s1-001", nextS1ClassDate);
  const s3SessaoId = createSessaoAulaId("agr-s3-001", nextS3ClassDate);
  ...
  const reservasAulas: ReservaAula[] = [
    {
      ...
      tenantId: TENANT_ID_S1,
      sessaoId: s1SessaoId,
      atividadeGradeId: "agr-s1-001",
      atividadeId: "atv-s1-002",
      atividadeNome: "Spinning",
      alunoId: "al-s1-001",
      alunoNome: "Bianca Rocha",
      data: nextS1ClassDate,
      horaInicio: "06:30",
      horaFim: "07:20",
      origem: "PORTAL_CLIENTE",
      status: "CONFIRMADA",
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:00:00`,
    },
    {
      id: "rva-s1-002",
      tenantId: TENANT_ID_S1,
      ...
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:01:00`,
    },
    {
      id: "rva-s1-003",
      tenantId: TENANT_ID_S1,
      ...
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:02:00`,
    },
    {
      id: "rva-s1-004",
      tenantId: TENANT_ID_S1,
      ...
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:03:00`,
    },
    {
      id: "rva-s1-005",
      tenantId: TENANT_ID_S1,
      ...
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:04:00`,
    },
    {
      id: "rva-s1-006",
      tenantId: TENANT_ID_S1,
      ...
      local: "Sala S1 Cardio",
      instrutorNome: "Priscila Gomes",
      dataCriacao: `${nextS1ClassDate}T06:05:00`,
    },
    {
      id: "rva-s3-001",
      tenantId: TENANT_ID_S3,
      sessaoId: s3SessaoId,
      atividadeGradeId: "agr-s3-001",
      atividadeId: "atv-s3-002",
      atividadeNome: "Ritmos",
      alunoId: "al-s3-001",
      alunoNome: "Thiago Prado",
      data: nextS3ClassDate,
      horaInicio: "19:00",
      horaFim: "19:50",
      origem: "PORTAL_CLIENTE",
      status: "CONFIRMADA",
      local: "Studio S3",
      instrutorNome: "Leandro Dias",
      dataCriacao: `${nextS3ClassDate}T18:00:00`,
    },
  ];
...
function normalizeStore(data: Store): Store {
  ...
  const result: Store = {
    ...
    crmPipelineStages: mergeById(data.crmPipelineStages, defaults.crmPipelineStages).map((stage) => ({
      ...stage,
      ativo: stage.ativo ?? true,
      slaHoras: stage.slaHoras ?? 24,
    })),
    crmTasks: mergeById(data.crmTasks, defaults.crmTasks).map((task) => ({
      ...task,
      origem: task.origem ?? "MANUAL",
      dataCriacao: task.dataCriacao ?? new Date().toISOString().slice(0, 19),
      dataAtualizacao: task.dataAtualizacao,
    })),
    crmPlaybooks: mergeById(data.crmPlaybooks, defaults.crmPlaybooks).map((playbook) => ({
      ...playbook,
      ativo: playbook.ativo ?? true,
      passos: Array.isArray(playbook.passos) ? playbook.passos : [],
      dataCriacao: playbook.dataCriacao ?? new Date().toISOString().slice(0, 19),
      dataAtualizacao: playbook.dataAtualizacao,
    })),
    crmCadencias: mergeById(data.crmCadencias, defaults.crmCadencias).map((cadencia) => ({
      ...cadencia,
      ativo: cadencia.ativo ?? true,
      passos: Array.isArray(cadencia.passos) ? cadencia.passos : [],
      dataCriacao: cadencia.dataCriacao ?? new Date().toISOString().slice(0, 19),
      dataAtualizacao: cadencia.dataAtualizacao,
    })),
    crmAutomations: mergeById(data.crmAutomations, defaults.crmAutomations).map((automation) => ({
      ...automation,
      ativo: automation.ativo ?? true,
      execucoes: automation.execucoes ?? 0,
      dataCriacao: automation.dataCriacao ?? new Date().toISOString().slice(0, 19),
      dataAtualizacao: automation.dataAtualizacao,
    })),
    crmActivities: mergeById(data.crmActivities, defaults.crmActivities).map((activity) => ({
      ...activity,
      dataCriacao: activity.dataCriacao ?? new Date().toISOString().slice(0, 19),
    })),
    reservasAulas: mergeById(data.reservasAulas, defaults.reservasAulas).map((reserva) => ({
      ...reserva,
      dataCriacao: reserva.dataCriacao ?? new Date().toISOString().slice(0, 19),
    ...
    nfseConfiguracoes: mergeById(data.nfseConfiguracoes, defaults.nfseConfiguracoes).map((config) => ({
      ...config,
      loteInicial: Math.max(1, Number(config.loteInicial ?? 1)),
      aliquotaPadrao: Number(config.aliquotaPadrao ?? 0),
      emissaoAutomatica: config.emissaoAutomatica ?? true,
      status: config.status ?? "PENDENTE",
    })),
    agregadorTransacoes: mergeById(data.agregadorTransacoes, defaults.agregadorTransacoes).map((item) => ({
      ...item,
      ...
      statusTransacao: item.statusTransacao ?? "PENDENTE",
      statusRepasse: item.statusRepasse ?? "PREVISTO",
      statusConciliacao: item.statusConciliacao ?? "PENDENTE",
    })),
    integracoesOperacionais: mergeById(data.integracoesOperacionais, defaults.integracoesOperacionais).map((item) => ({
      ...item,
      filaPendente: Math.max(0, Number(item.filaPendente ?? 0)),
      ocorrencias: Array.isArray(item.ocorrencias) ? item.ocorrencias : [],
      status: item.status ?? "CONFIGURACAO_PENDENTE",
    })),
