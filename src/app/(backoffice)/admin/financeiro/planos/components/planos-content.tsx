...
interface PlanosContentProps {
  initialData: PlanoPlataforma[];
}

export function PlanosContent({ initialData }: PlanosContentProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const planosQuery = useAdminPlanos();
  const loading = planosQuery.isLoading && !initialData.length;
  const error = planosQuery.error ? normalizeErrorMessage(planosQuery.error) : null;
  const planos = planosQuery.data ?? initialData;
