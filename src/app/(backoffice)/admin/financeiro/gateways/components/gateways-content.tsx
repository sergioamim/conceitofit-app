...
interface GatewaysContentProps {
  initialData: GatewayPagamento[];
}

export function GatewaysContent({ initialData }: GatewaysContentProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const gatewaysQuery = useAdminGateways();
  const loading = gatewaysQuery.isLoading && !initialData.length;
  const error = gatewaysQuery.error ? normalizeErrorMessage(gatewaysQuery.error) : null;
  const gateways = gatewaysQuery.data ?? initialData;
