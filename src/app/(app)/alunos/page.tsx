...
import { listAlunos, listPlanos, listFormasPagamento, criarAlunoComMatricula } from "@/lib/mock/services";
...
function NovoAlunoWizard({ open, onClose, onDone }: { open: boolean; onClose: () => void; onDone: () => void }) {
...
export default function AlunosPage() {
  ...
  return (
    <div className="space-y-6">
      <NovoAlunoWizard open={wizardOpen} onClose={() => setWizardOpen(false)} onDone={load} />
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-secondary">
              {["Aluno", "CPF", "Telefone", "Nascimento", "Sexo", "Status"].map((h) => (
          ...
          <tbody className="divide-y divide-border">
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="py-10 text-center text-sm text-muted-foreground">Nenhum aluno encontrado</td></tr>
            )}
            {filtered.map((a) => {
