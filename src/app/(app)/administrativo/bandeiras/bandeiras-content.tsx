...
interface BandeirasContentProps {
  initialData: BandeiraCartao[];
}

export function BandeirasContent({ initialData }: BandeirasContentProps) {
  const { confirm, ConfirmDialog } = useConfirmDialog();
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<BandeiraCartao | null>(null);

  const { items: bandeiras, error, reload, handleSave, handleToggle, handleDelete } = useCrudOperations<BandeiraCartao>({
    listFn: listBandeirasCartaoApi,
    createFn: (data: any) => createBandeiraCartaoApi(data),
    updateFn: (id, data: any) => updateBandeiraCartaoApi({ id, ...data }),
    toggleFn: toggleBandeiraCartaoApi,
    deleteFn: deleteBandeiraCartaoApi,
    initialData,
  });

  const onSave = async (data: Omit<BandeiraCartao, "id">, id?: string) => {
    await handleSave(data as never, id);
    setModalOpen(false);
    setEditing(null);
  };

  const onDelete = (id: string) => {
    confirm("Remover esta bandeira?", () => handleDelete(id));
  };
