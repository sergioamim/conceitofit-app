    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <ClienteHeader
          ...
          onCompletarCadastro={() => w.setTab("editar")}
          ...
          onEdit={() => w.setTab("editar")}
          onChangeFoto={() => w.setPhotoModalOpen(true)}
        />
        {w.liberarAcessoInfo && <div className="mt-3 rounded-xl border border-gym-accent/40 bg-gym-accent/10 p-3 text-sm text-gym-accent">{w.liberarAcessoInfo}</div>}
        {w.actionError && <div className="mt-3 rounded-xl border border-gym-danger/40 bg-gym-danger/10 p-3 text-sm text-gym-danger">{w.actionError}</div>}
        {w.migracaoResumo && (
      ...
      <ClienteTabs current={w.tab} baseHref={`/clientes/${aluno.id}`} onSelect={(next) => w.setTab(next)} pendenteFinanceiro={w.pendenteFinanceiro} showEditTab={w.tab === "editar"} />
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-foreground">{w.clienteContexto?.tenantName ?? w.tenantCatalogo.get(aluno.tenantId)?.nome ?? aluno.tenantId}</p>
            </div>
          </div>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-2 text-sm text-foreground">
            {w.clienteContexto?.eligibleTenants.length ? w.clienteContexto.eligibleTenants.map((t) => t.tenantName ?? t.tenantId).join(" • ") : "Sem unidades elegíveis informadas"}
          </p>
          {w.clienteContexto?.blockedTenants.length ? (
            <p className="mt-2 text-xs text-amber-300">Bloqueios: {w.clienteContexto.blockedTenants.map((t) => t.tenantName ?? t.tenantId).join(" • ")}</p>
          ) : null}
        </div>
      </div>
      ...
      {w.tab === "resumo" && (
        <>
          <div className="grid grid-cols-4 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Dados principais</p>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Button variant="outline" onClick={() => w.router.push(`/pagamentos?clienteId=${aluno.id}`)} className="border-border text-xs">Ver todas</Button>
            </div>
            <div className="divide-y divide-border">
              {w.vendas.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Nenhuma venda encontrada</p>}
              {w.vendas.map((p) => (
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="mt-3 divide-y divide-border">
              {(aluno.suspensoes?.length ?? 0) === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Nenhuma suspensão registrada</p>}
      ...
      {w.tab === "editar" && (
        <div className="rounded-xl border border-border bg-card p-5">
          <ClienteEditForm aluno={aluno} onCancel={() => w.setTab("resumo")} onSaved={async () => { await w.reload(); w.setTab("resumo"); }} />
        </div>
      )}
