...
export default function CadastroPublicoPage() {
  ...
  const [form, setForm] = useState<PublicSignupDraft>(DEFAULT_SIGNUP);
  ...
  useEffect(() => {
    if (!context) return;
    setForm(draft.signup ?? DEFAULT_SIGNUP);
    setSelectedPlanId(planId ?? draft.planId ?? context.planos[0]?.id ?? "");
  }, [context, draft.planId, draft.signup, planId]);
    ...
    <PublicJourneyShell
      ...
      actions={
        <Button asChild variant="outline" className="border-border bg-transparent">
          <Link href={buildPublicJourneyHref("/adesao", { tenantRef: resolvedTenantRef, planId: selectedPlanId })}>
            Voltar aos planos
          </Link>
        </Button>
      }
