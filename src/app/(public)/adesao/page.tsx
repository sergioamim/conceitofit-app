      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <Card className="max-w-xl border-rose-500/40 bg-rose-500/10">
          <CardContent className="px-6 py-5 text-sm text-rose-100">
            {error || "Não foi possível carregar a jornada pública."}
          </CardContent>
        </Card>
      </div>
    );
      ...
      <section className="mt-6 grid gap-4 xl:grid-cols-3">
        {context.planos.map((plan) => {
          ...
          return (
            <Card
              key={plan.id}
              className={`border-border/70 bg-card/70 backdrop-blur ${plan.destaque ? "ring-1 ring-gym-accent/40" : ""}`}
            >
              <CardContent className="px-6 py-6">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-3 font-display text-2xl font-bold">{plan.nome}</h3>
                  </div>
                  {plan.destaque ? (
                ...
                <div className="mt-6">
                  <p className="font-display text-4xl font-bold text-gym-accent">{formatCurrency(plan.valor)}</p>
                  <p className="mt-2 text-sm text-muted-foreground">Total inicial estimado: {formatCurrency(quote.total)}</p>
                </div>

                <div className="mt-6 space-y-3">
                  {quote.items.map((item) => (
                    <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-border/70 bg-secondary/25 px-3 py-2">
                      <div>
