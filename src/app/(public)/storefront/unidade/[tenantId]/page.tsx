...
import {
  getStorefrontUnidadeDetalhe,
  type StorefrontUnidadeDetalhadaResponse,
...
async function fetchUnidade(academiaSlug: string, tenantId: string) {
  try {
    const data = await getStorefrontUnidadeDetalhe(academiaSlug, tenantId);
    return { unidade: data, planos: data.planos ?? [] };
  } catch (error) {
    logger.warn("[Storefront/Unidade] Fetch failed", { error, academiaSlug, tenantId });
    return { unidade: null as StorefrontUnidadeDetalhadaResponse | null, planos: [] as StorefrontPlanoPublicoResponse[] };
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { tenantSlug: rawSlug, academiaSlug } = await resolveStorefrontHeaders(tenantId);
...
export default async function UnidadePage({ params }: PageProps) {
  const { tenantId } = await params;
  const { resolveStorefrontHeaders } = await import("../../resolve-storefront-headers");
  const { academiaSlug } = await resolveStorefrontHeaders(tenantId);

  const { unidade, planos } = await fetchUnidade(academiaSlug, tenantId);

  if (!unidade) {
    return (
      <main className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-6 text-center">
        <h1 className="font-display text-2xl font-bold">Unidade não encontrada</h1>
        <p className="max-w-md text-sm text-muted-foreground">
          Não foi possível carregar os dados desta unidade. Verifique se o backend está disponível e se o tenant <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">{tenantId}</code> existe.
        </p>
      </main>
    );
  }
      ...
      {unidade.horarioFuncionamento && unidade.horarioFuncionamento.length > 0 && (
        <section className="mx-auto mt-10 max-w-6xl px-6">
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {unidade.horarioFuncionamento.map((horario) => (
              <div
                key={horario.dia}
                className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-2.5"
              >
                <span className="text-sm font-medium">
                  {DIAS_SEMANA[horario.dia.toUpperCase()] ?? horario.dia}
                </span>
