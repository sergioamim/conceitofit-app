...
- Fato observado no contrato publicado em `/Users/sergioamim/dev/pessoal/academia-java/modulo-app/src/main/resources/static/openapi.yaml`: `POST /api/v1/admin/integracoes/importacao-terceiros/evo/p0/pacote/{uploadId}/job` referencia `CriarJobPacoteImportacaoRequest`, com `dryRun`, `maxRejeicoesRetorno`, `arquivos`, `tenantId`, `evoUnidadeId` (`integer/int32`) e `apelido`.
- Fato observado em worktrees ativos do backend, por exemplo `/Users/sergioamim/dev/pessoal/academia-java/worktrees/task-396/modulo-importacao-terceiros/src/main/java/fit/conceito/importacao/controller/ImportacaoTerceirosController.java`: o endpoint alias também consome `tenantId` e `evoUnidadeId` no body e valida acesso por `X-Context-Id` ou `X-Tenant-Id`.
- Fato observado no checkout principal `main` do backend em `/Users/sergioamim/dev/pessoal/academia-java/modulo-app/src/main/java/fit/conceito/app/application/controller/admin/AdminImportacaoEvoController.java`: existe uma implementação administrativa paralela do create job cujo DTO não declara `tenantId` nem `evoUnidadeId`.
- Pendência/lacuna: há divergência real entre o checkout principal do backend e o contrato/runtime do módulo `importacao-terceiros` presente nas worktrees. O frontend foi mantido alinhado ao contrato alias usado pelo runtime que gera os logs do EVO, exigindo `tenantId` explícito e `evoUnidadeId` inteiro válido no body para evitar `tenant_context ausente` e `api_payload_invalid`.
